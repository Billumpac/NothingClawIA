...
Singleton {
    ...
    // Screen Receiver (incoming stream viewer) state
    property bool screenReceiverVisible: false
    property string screenReceiverUrl: ""
    property int screenReceiverFps: 15
    property string screenReceiverPin: ""
    property string screenReceiverSource: ""   // friendly name of the source
    property bool screenReceiverFullscreen: false

    // Mirror Tool state (camera mirror)
    property bool mirrorWindowVisible: false

    // Desktop Mirror Tool state (live screen clone via ScreencopyView)
    property bool desktopMirrorWindowVisible: false
    property string desktopMirrorSourceScreenName: ""
    ...
    function toggleAssistant() {
        if (assistantVisible) {
            // Already open: if the current screen is the one hosting it,
            // close. Otherwise move it to the focused screen.
            if (assistantScreenName
