...
Singleton {
    ...
    // Controls
    function toggleMute() {
        if (!_muteDebounceTimer.running) {
            if (sink?.audio) {
                sink.audio.muted = !sink.audio.muted;
            }
        }
        _muteDebounceTimer.restart();
    }
    ...
    Timer {
        id: _muteDebounceTimer
        interval: 40
        repeat: false
    }

    function incrementVolume() {
        if (!_volumeDebounceTimer.running) {
            if (sink?.audio) {
    ...
    // Debounce: ignores follow-up calls within 40ms so a single key press
    // only changes volume once even if the bind fires twice (e.g. duplicate
    // registration between axctl.toml and live keybinds-batch). Below human
    // perception, so a real double-press is unaffected.
    Timer {
        id: _volumeDebounceTimer
        interval: 40
        repeat: false
    }
